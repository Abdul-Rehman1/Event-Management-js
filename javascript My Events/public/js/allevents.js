...
function render(events){
   if(events.goingId.indexOf(JSON.parse(sessionStorage.getItem('Eventp_userInfo')).email)==-1){
        ...
        discription.appendChild(document.createTextNode(events.eventDesc));
        ...
        goingAnchor.appendChild(document.createTextNode("Going"));
        goingAnchor.setAttribute("class","card-link");
        goingAnchor.setAttribute("href","javaScript:void(0)");
        goingAnchor.onclick=function(){ fnGoing(events)  }
...
function fnGoing(events){
    ...
    going.push(JSON.parse(sessionStorage.getItem('Eventp_userInfo')).email);  
    ...
    modalboxAlert("Event added to My Events","green");
